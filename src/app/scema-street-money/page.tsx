...
import { Column, detailScemaUangJalanAside } from './Column'
import moment, { now } from 'moment'
import { IScemaUangJalan } from './InterfaceDataScema'
...
export default function ScemaStreet() {
    ...
    const { deleteData } = useMultyDestory(`uang-jalan/multy-destory`)
    const router = useRouter()

    const hndelDeleted = (row?: IScemaUangJalan) => {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                const isData = row ? [row?.id ?? null] : dataRowClicick.map((d: any) => d.id)
                deleteData({
                    data: isData,
                    onSuccess: () => {
            ...
            <div className="mt-20 pt-10">
                <div className="row row-cols-3">
                    ...
                    <div className="col-12 sm:col-12 md:col-12 xl:col-3">
                        <CardInfoDetails title={dataRowClicick?.kode_scema ?? ''} titleColor="xl:text-gray-100" list={detailScemaUangJalanAside(dataRowClicick)}>
